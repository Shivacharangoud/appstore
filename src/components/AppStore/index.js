...
class AppStore extends Component {
  ...
  render() {
    ...
    const smallSearchInput = search.toLowerCase()

    const filteredApps = appsList.filter(each => {
      const categoryMatch = each.category === category
      const appNameSmall = each.appName.toLowerCase()
      const nameMatch = appNameSmall.includes(smallSearchInput)
      const totalMatch = categoryMatch && nameMatch
      return totalMatch
    })
    return (
      <div className="bg-container">
        <h1 className="heading">App Store</h1>
        <div className="input-logo-con">
          <input
